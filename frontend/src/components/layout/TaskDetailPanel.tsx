...
export function TaskDetailPanel({ onTaskUpdate }: TaskDetailPanelProps) {
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      setIsUpdating(true)
      const newStatus = selectedTask.status === TaskStatus.DONE ? TaskStatus.TODO : TaskStatus.DONE
      ...
      const updatedTask = { ...selectedTask, status: newStatus, completed_percentage: newStatus === TaskStatus.DONE ? 100 : 0 }
      setSelectedTask(updatedTask)
  ...
  const getStatusColor = (status: TaskStatus) => {
    switch (status) {
      case TaskStatus.BACKLOG: return 'gray'
  ...
  const fetchComments = async (taskId: string) => {
    try {
      ...
      const response = await fetch(`${baseUrl}/api/v1/tasks/${taskId}/comments`, {
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch comments:', response.status, await response.text())
      }
    } catch (error) {
  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      const baseUrl = import.meta.env.VITE_API_URL || 'http://localhost:9200'
      const response = await fetch(`${baseUrl}/api/v1/tasks/${selectedTask.id}/comments`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('auth_token')}`,
          'Content-Type': 'application/json',
      ...
      <Box px={6} py={4} borderBottom="1px" borderBottomColor="gray.100">
        ...
        <HStack justify="space-between" mb={3}>
          <HStack spacing={2}>
            ...
            <Button variant="ghost" size="sm" p={1.5} minW={6} h={7} onClick={handleShare}>
              <Icon as={FiShare2} boxSize={3.5} color="gray.500" />
            </Button>
            <Button variant="ghost" size="sm" p={1.5} minW={6} h={7} onClick={handleAttach}>
              <Icon as={FiPaperclip} boxSize={3.5} color="gray.500" />
            </Button>
            <Button variant="ghost" size="sm" p={1.5} minW={6} h={7} onClick={handleExpand}>
              <Icon as={FiMaximize2} boxSize={3.5} color="gray.500" />
            </Button>
            <Button variant="ghost" size="sm" p={1.5} minW={6} h={7} onClick={() => handleMenuAction("Edit")}>
              <Icon as={FiEdit3} boxSize={3.5} color="gray.500" />
            </Button>
            <Menu>
              <MenuButton as={Button} variant="ghost" size="sm" p={1.5} minW={6} h={7}>
                <Icon as={FiMoreHorizontal} boxSize={3.5} color="gray.500" />
              </MenuButton>
              <MenuList>
                <MenuItem onClick={() => handleMenuAction("Edit")}>Edit Task</MenuItem>
                <MenuItem onClick={() => handleMenuAction("Duplicate")}>Duplicate</MenuItem>
                <MenuItem onClick={() => handleMenuAction("Archive")}>Archive</MenuItem>
                <MenuItem onClick={() => handleMenuAction("Delete")}>Delete</MenuItem>
              </MenuList>
            </Menu>
          </HStack>
          <Button variant="ghost" size="sm" p={1.5} minW={6} h={7} onClick={() => setSelectedTask(null)}>
      ...
      <Box flex={1} overflow="auto">
          ...
          <Box px={6} py={3} borderBottom="1px" borderBottomColor="gray.50">
            <HStack justify="space-between" align="center">
              ...
              <HStack spacing={2}>
                <Avatar 
                  size="sm" 
                  name={selectedTask.assignee.full_name || selectedTask.assignee.username}
        ...
        <Box px={6} py={4}>
          ...
          <HStack justify="space-between" align="center" py={2}>
            ...
            <Badge 
              bg={selectedTask.status === TaskStatus.IN_PROGRESS ? "blue.500" : selectedTask.status === TaskStatus.DONE ? "green.500" : "gray.500"}
              color="white" 
              fontSize="xs" 
              px={3} 
              py={1.5} 
          ...
          <HStack justify="space-between" align="center" py={2}>
            ...
            <Badge 
              bg={selectedTask.priority === TaskPriority.HIGH ? "orange.50" : selectedTask.priority === TaskPriority.LOW ? "gray.50" : "blue.50"}
              color={selectedTask.priority === TaskPriority.HIGH ? "orange.700" : selectedTask.priority === TaskPriority.LOW ? "gray.600" : "blue.700"}
              fontSize="xs" 
              px={3} 
              py={1.5} 
              borderRadius="md"
        ...
        <Box px={6} py={4} borderBottom="1px" borderBottomColor="gray.50">
          ...
          {isEditingTask ? (
            ...
            <Text fontSize="sm" color="gray.700" lineHeight="1.6" bg="gray.50" p={3} borderRadius="md">
        ...
        <Box px={6} py={4}>
          {/* Tab Headers */}
          <HStack spacing={8} mb={4} borderBottom="1px" borderBottomColor="gray.100" pb={2}>
            <Box position="relative" cursor="pointer" onClick={() => setActiveTab("comments")}>
              <Text 
                fontSize="sm" 
            ...
            <Box position="relative" cursor="pointer" onClick={() => setActiveTab("updates")}>
              <Text 
                fontSize="sm" 
          ...
          <VStack spacing={4} align="stretch" mb={4}>
            {activeTab === "comments" ? (
              isLoadingComments ? (
                ...
                comments.map((comment) => (
                  <HStack key={comment.id} align="start" spacing={3}>
                    <Avatar size="sm" name={comment.user_name} bg="purple.500" color="white" />
              ...
              <Text fontSize="sm" color="gray.500" textAlign="center" py={4}>
                Activity updates will appear here when changes are made to this task.
              </Text>
            )}
          </VStack>
